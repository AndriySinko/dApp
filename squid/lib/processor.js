...
const CHALLENGE_TYPE_MAP = {
    0: model_1.ChallengeType.INDIVIDUAL,
    1: model_1.ChallengeType.GROUP,
    2: model_1.ChallengeType.PUBLIC,
};
const VERIFIER_MAP = {
    0: model_1.VerifierType.ON_CHAIN,
    1: model_1.VerifierType.API_ORACLE,
    2: model_1.VerifierType.AI_ORACLE,
};
const STATE_MAP = {
    0: model_1.ChallengeState.JOIN_OPEN,
    1: model_1.ChallengeState.ACTIVE,
    2: model_1.ChallengeState.VERIFY_PENDING,
    3: model_1.ChallengeState.SETTLED,
};
const ACTIVITY_MAP = {
...
// ─── Batch handler ────────────────────────────────────────────────────────────
processor.run(new typeorm_store_1.TypeormDatabase({ supportHotBlocks: true }), async (ctx) => {
    ...
    const challengeAddrsNeeded = new Set();
    const participantKeysNeeded = new Set();
    ...
    const existingChallenges = challengeAddrsNeeded.size > 0
        ? await ctx.store.findBy(model_1.Challenge, { id: (0, typeorm_1.In)([...challengeAddrsNeeded]) })
        : [];
    const existingParticipants = participantKeysNeeded.size > 0
        ? await ctx.store.findBy(model_1.Participant, { id: (0, typeorm_1.In)([...participantKeysNeeded]) })
        : [];
    const challenges = new Map(existingChallenges.map(c => [c.id, c]));
    const participants = new Map(existingParticipants.map(p => [p.id, p]));
    const bets = [];
    const repUpdates = [];
        ...
        for (const log of block.logs) {
            ...
            if (a === FACTORY_ADDRESS) {
                if (t === topics.ChallengeCreated) {
                    ...
                    const [typeNum, verifierNum, title, criteria, buyIn, joinDeadline, challengeDeadline] = abiCoder.decode(["uint8", "uint8", "string", "string", "uint256", "uint256", "uint256"], log.data);
                    const c = challenges.get(contractAddr) ?? new model_1.Challenge({ id: contractAddr });
                    c.challengeId = challengeId;
                    c.type = CHALLENGE_TYPE_MAP[Number(typeNum)];
                    c.verifier = VERIFIER_MAP[Number(verifierNum)];
                    c.state = model_1.ChallengeState.JOIN_OPEN;
                    c.creator = creator;
                    c.title = title;
                    c.criteria = criteria;
                    c.buyIn = BigInt(buyIn);
                    c.joinDeadline = BigInt(joinDeadline);
                    c.challengeDeadline = BigInt(challengeDeadline);
                    c.createdAt = ts;
                    c.createdTxHash = tx;
                    ...
                    ctx.log.info(`ChallengeCreated: ${contractAddr} "${title}"`);
                }
            ...
            if (a === REPUTATION_ADDRESS && t === topics.ReputationUpdated) {
                ...
                const ru = new model_1.ReputationUpdate({ id: `${tx}-${li}` });
                ru.user = user;
                ru.delta = BigInt(delta);
                ru.newScore = BigInt(newScore);
                ru.challenge = challenge;
                ru.timestamp = ts;
                repUpdates.push(ru);
                const ur = userReps.get(user) ?? new model_1.UserReputation({ id: user });
                ur.score = BigInt(newScore);
                ur.lastUpdated = ts;
            ...
            if (a === GOVERNANCE_ADDRESS && t === topics.EpochTicked) {
                const [epoch, , winningTitle, publicChallengeAddress, prizePool] = abiCoder.decode(["uint256", "uint256", "string", "address", "uint256"], log.data);
                const er = new model_1.EpochResult({ id: BigInt(epoch).toString() });
                er.epoch = BigInt(epoch);
                er.winningTitle = winningTitle;
                er.publicChallengeAddress = publicChallengeAddress.toLowerCase();
                er.prizePool = BigInt(prizePool);
                er.timestamp = ts;
            ...
            if (t === topics.BetPlaced) {
                ...
                const [side, amount, newForPool, newAgainstPool] = abiCoder.decode(["bool", "uint256", "uint256", "uint256", "uint256"], log.data);
                const bet = new model_1.Bet({ id: `${tx}-${li}` });
                bet.challenge = c;
                bet.bettor = bettor;
                ...
                bet.txHash = tx;
                bets.push(bet);
                c.forPool = BigInt(newForPool);
                c.againstPool = BigInt(newAgainstPool);
                if (Boolean(side))
                    c.bettorsFor = (c.bettorsFor ?? 0) + 1;
                else
                    c.bettorsAgainst = (c.bettorsAgainst ?? 0) + 1;
            }
                ...
                const p = participants.get(key) ?? new model_1.Participant({ id: key });
                p.challenge = c;
                p.participant = participant;
                p.stake = BigInt(stake);
                p.joinedAt = ts;
